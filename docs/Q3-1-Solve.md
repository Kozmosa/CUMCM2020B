...
第五关没有村庄，因此对手现金不会影响其未来行动可行性、资源消耗、位置或存活状态。最佳反应状态键可安全删除对手现金，只保留偏离者位置/库存以及两名对手的活动标记、位置和库存；偏离者现金仍作为同键下需要最大化的值。

当前实现把这些字段编码为 67 位紧凑键，并由 Numba 开放寻址哈希表在生成后继时直接完成去重和最大现金更新。父指针只保存上一层整数行号和一字节行动编码。正式第五关基准单次 BR 的最大前沿约为 $1.14\times10^6$，累计处理约 $8.75\times10^6$ 个状态，峰值 RSS 约 397 MiB；不再构造数百万个 Python 元组和 dataclass 对象。

以第一个确定性 Gauss--Seidel 更新顺序得到并经三次完整最佳反应重新认证的策略为：

| 玩家 | 第 0 天购买 | 行动序列（到达终点前） | 到达日 | 支付/元 | regret/元 |
|---|---|---|---:|---:|---:|
| 1 | 水 27、食物 33 | $1\to4$，停留，$4\to6\to13$ | 4 | 9535 | 0 |
| 2 | 水 30、食物 37 | 停留，停留，$1\to5\to6\to13$ | 5 | 9480 | 0 |
| 3 | 水 30、食物 34 | $1\to5\to6\to13$ | 3 | 9510 | 0 |

因此该策略组合状态为 `CERTIFIED_PURE`。`selection_complete=false` 仅表示尚未枚举并比较所有其他纯均衡，不影响上述组合的纳什不等式认证。
