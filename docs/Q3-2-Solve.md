...
- Q3.2 `exact|adaptive` 双后端、确定性购买候选、完整纯偏离扫描、资源感知向上舍入上界和 NashConv 混合兜底；
- `PolicyEntry`、`SolveReport` 和明确的均衡状态；
- v2 目录检查点（manifest、逐日 NumPy 层、阶段元数据）以及 v1 pickle 迁移读取；
- 墙钟/RSS/状态/profile 预算和原子检查点。

小规模穷举与回归测试已覆盖标量/批量转移、偏离导致对手提前失败、上界支配、exact/adaptive 一致性、无纯均衡混合解以及 v1/v2 恢复。完整第六关仍应按 24 小时、256 GiB 的正式命令运行；预算耗尽时输出 `SEARCH_STOPPED` 和可恢复检查点，不把有限候选结果伪装成认证均衡。
