...
>
> 本文只采用无损状态压缩、保守的确定性剪枝以及不改变结果的向量化和 CPU 并行化。本文不使用资源量化、Beam Search、低概率天气截断、Top-K 购买、近似价值函数或其他有损搜索。
...
首版数值求解器以**纯策略马尔可夫完美均衡**为目标。对于每个时间、公开联合状态和当天已观察天气，求纯策略阶段纳什均衡联合行动，并通过逆向归纳得到后续价值。

如果某个阶段博弈不存在纯策略纳什均衡，求解器应明确返回“该状态不存在纯策略均衡”，而不是静默采用近似行动或截断搜索。混合策略均衡不属于首版实现范围。
...
- 按主行动、目的地和是否购买分组的无损结构化联合行动枚举；
- 小阶段密集支付张量与大阶段分块最佳反应搜索的自动切换；
- 块内后继状态规范化和精确去重；
- 相同玩家状态的个人行动集合共享；
- 第 0 天基于乐观可达性的严格支配购买删除；
- 可配置线程数、线程安全状态缓存和进行中状态等待；
- 原子检查点、阶段对手行动对偏移保存和跨进程恢复；
- CPython 3.13 与自由线程 CPython 3.13t 的等价性测试。

第六关第 0 天从每人 120601 个购买动作无损缩减为 88925 个，但三人有序联合动作仍约为

$$
88925^3=703188276453125.
$$

因此分块后端解决的是内存有界和可恢复问题，并不单独消除最坏时间复杂度；继续扩大完整计算仍需加入带证明的单人乐观价值上界和最佳反应下界剪枝。
